...
// Program to calculate Pearsons using omp guided scheduler
#include <omp.h>
void runParallelGuided(int size, int thread_count, double timeTakenSerial)
{
    //Run guided Version
    printf("------ParallelGuided------\n");
    fflush(stdout);
    ...
    fflush(stdout);
    printf("Time taken for guided Version was %lf\n",t2);
    if(timeTakenSerial!=0)
    {
        if(t2<timeTakenSerial)
        {
            printf("Guided was %.1lf%% quicker\n\n",((timeTakenSerial/t2)-1)*100);
        }
        else
        {
            printf("Guided was %.1lf%% slower\n\n",((t2/timeTakenSerial)-1)*100);
        }
    }
    else
    {
