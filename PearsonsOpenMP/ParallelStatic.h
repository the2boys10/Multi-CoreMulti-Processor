...
// Program to calculate Pearsons using omp static scheduler
#include <omp.h>
void runParallelStatic(int size, int thread_count, double timeTakenSerial)
{
    //Run Static Version
    printf("------ParallelStatic------\n");
    fflush(stdout);
    ...
    // fill our array in parallel using static schedule
    ...
    double sumOfA = 0;
    double sumOfB = 0;
    // work out the sum in parallel using static scheduling
    #pragma omp parallel for schedule(static) reduction(+:sumOfB) reduction(+:sumOfA) num_threads(thread_count)
    for (int i = 0; i < size; i++)
    {
    ...
    fflush(stdout);
    printf("Time taken for static Version was %lf\n",t2);
    if(timeTakenSerial!=0)
    {
        if(t2<timeTakenSerial)
        {
            printf("Static was %.1lf%% quicker\n\n",((timeTakenSerial/t2)-1)*100);
        }
        else
        {
            printf("Static was %.1lf%% slower\n\n",((t2/timeTakenSerial)-1)*100);
        }
    }
    else
    {
